...
WINDOW_ICONS = {
    ...
    'jetbrains-toolbox': fa.icons['code'],
    ...
    'nmtui': fa.icons['wifi'],
    'float_calcurse': fa.icons['calendar'],
    'float_calc': fa.icons['calculator'],
}
...
def term_icon_for_window(window):
    names = xprop(window.window, 'WM_NAME')
    if names != None and len(names) > 0:
        for nm in names:
            nm = nm.lower()
            if nm in TERMINAL_ICONS:
                return TERMINAL_ICONS[nm]
    logging.info(
        'No icon available for window with names: %s' % str(names))
    return WINDOW_ICONS['st']
